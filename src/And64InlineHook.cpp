...
static int __make_rwx(void* p, size_t n)
{
    uintptr_t start_addr_align = ALIGN_DOWN((uintptr_t)p, PAGE_SIZE);
    size_t size = 0;
    ...
    return mprotect((void*)start_addr_align, size, PROT_READ | PROT_WRITE | PROT_EXEC);
}
...
{
    uint_fast64_t mask = 0x03ffffffu; // 0b00000011111111111111111111111111

    uint32_t* trampoline = (uint32_t*)rwx;
    uint32_t* original = (uint32_t*)symbol;

    if (A64_MAX_INSTRUCTIONS < 5)
    {
        LOGE("please fix A64_MAX_INSTRUCTIONS!");
        exit(-1);
    }
    ...
    if (llabs(pc_offset) >= (mask >> 1))
    {
        int32_t count = ((uint64_t)(original + 2) & 7u) != 0u ? 5 : 4;
        if (rwx_size < count * 10u)
        {
            LOGE("rwx size is too small to hold %u bytes backup instructions!", count * 10u);
            return NULL;
        }
    ...
    else
    {
        if (rwx_size < 1u * 10u)
        {
            LOGE("rwx size is too small to hold %u bytes backup instructions!", 1u * 10u);
            return NULL;
        }
...
void A64HookFunction(void* const symbol, void* const replace, void** result)
{
    ...
    __make_rwx(trampoline, A64_MAX_INSTRUCTIONS * 10);
    *result = trampoline;
